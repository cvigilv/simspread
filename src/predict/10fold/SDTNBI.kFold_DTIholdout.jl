...
"""
    createAdjMatrix(DT::AbstractMatrix, DS::AbstractMatrix)

Create tri-layered substructure-drug-target network adjacency matrix.

# Arguments
- `DT::AbstractMatrix`: Drug-Target rectangular adjacency matrix.
- `DS::AbstractMatrix`: Drug-Substructure rectangular adjacency matrix.
"""
...
Create tri-layered compound-substructure-drug-target network adjacency matrix.

# Arguments
- `DT::AbstractMatrix`: Drug-Target rectangular adjacency matrix.
- `DS::AbstractMatrix`: Drug-Substructure rectangular adjacency matrix.
- `C::AbstractArray`: Compounds vector.
"""
...
Prepare adjacency matrix for cross-validation splitting.

# Arguments
- `DT₀::AbstractMatrix`: Drug-Target rectangular adjacency matrix.
- `DS₀::AbstractMatrix`: Drug-Substructure rectangular adjacency matrix.
- `Eᵢ::AbstractArray`: Edges to remove.
"""
...
"""
    predict(A::NamedArray, B::NamedArray)

DTI prediction using de novo NBI over SDT network

# Arguments
- `A::NamedArray`: SDT adjacency matrix.
- `B::NamedArray`: SDT adjacency matrix.
"""
...
predict(A::Nothing, B::Nothing) = nothing

"""
    clean(A::NamedArray, B::NamedArray)

Clean predictions

# Arguments
- `R::NamedArray`: DTI predictions matrix.
- `A::NamedArray`: SDT adjacency matrix.
"""
...
clean(R::Nothing, A::Nothing) = nothing
clean(R::NamedArray, A::Nothing) = nothing
clean(R::Nothing, A::NamedArray) = nothing

"""
	save(R::NamedArray, DT::NamedArray, E::Array, foldᵢ::Int64, fout::String)

Save predictions

# Arguments
- `R::NamedArray`: DTI predictions matrix.
- `DT::NamedArray`: DT adjacency matrix.
- `E::Array`: Deleted edges.
- `foldᵢ::Int64`: Fold ID.
